...
public class SlotMachine extends Slot {
	...
	public SlotMachine(EntityPlayer par1EntityPlayer,
	...
	protected void onCrafting(ItemStack par1ItemStack) {
		...
		if (!this.thePlayer.worldObj.isRemote) {
			...
			if (f == 0.0F || g == 0.0f) {
				i = 0;
			} else if (f < 1.0F || g < 1.0f) {
				j = MathHelper.floor_float((float) i * f);
				j = MathHelper.floor_float((float) i * g);

				if (j < MathHelper.ceiling_float_int((float) i * f)
						&& (float) Math.random() < (float) i * f - (float) j
						|| j < MathHelper.ceiling_float_int((float) i * g)
						&& (float) Math.random() < (float) i * g - (float) j) {
					++j;
				}
